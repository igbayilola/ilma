...
**✅ Robustesse Dashboard** (iter 29) : application du pattern iter 28 au Dashboard élève (`pages/Dashboard.tsx`), qui partageait la même faille `listSubjects(undefined)` et n'avait aucune garde `gradeLevelId`. L'effet skip le fetch sans classe, nouvelle branche `NoClassEmptyState` (icône `GraduationCap` + CTA `/select-profile`) rendue avant `isLoading`, et flag `cancelled` dans le useEffect pour éviter les setState après unmount sur les trois fetch chaînés (subjects → skills → progress). Le `EmptyStateDashboard` existant (« télécharge un pack hors-ligne ») est conservé : c'est une intention produit différente, pas le même cas réseau. Premier fichier de test `dashboardPage.test.tsx` posé avec la mock surface complète (auth + app store + content/progress/exam services + useNavigate) — 2 cas (garde sans classe, smoke avec classe). Total vitest : 124/124, 18 fichiers.

**✅ Couverture rendu ProgramTimeline** (iter 30) : la fn pure `groupByTrimesterWeek` était testée depuis iter 22 (7 cas dans `programTimeline.test.ts`), mais le composant React `<ProgramTimeline>` (183 LOC) n'avait aucun test de rendu. `programTimelineRender.test.tsx` mocke `groupByTrimesterWeek` + `getCurrentTrimesterWeek` pour alimenter directement `TimelineTrimester[]` et couvre 8 cas : fallback « programme indisponible » (allEmpty), ligne « Tu es actuellement en … » présente/masquée selon le calendrier, badge « en cours » sur le trimestre courant (scopé pour éviter le faux positif sur la stat « X en cours »), message dédié pour un trimestre sans semaines, et `SkillRow` × 3 (clic non-future navigue, statut future `disabled` + `aria-disabled`, score `inProgress` affiché). Vitest : 132/132, 19 fichiers.
