...
export const ProfileSelectorPage: React.FC = () => {
    ...
    if (isLoading) {
        return (
            <div className="min-h-screen bg-ilma-surface flex items-center justify-center p-8">
    ...
    return (
        <div className="min-h-screen bg-ilma-surface flex flex-col items-center justify-center p-8">
                ...
                <div className="flex flex-wrap gap-6 justify-center mb-10">
                    {profiles.filter(p => p.isActive).map((profile) => (
                        <button
                            ...
                            className="group flex flex-col items-center w-32 transition-transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-ilma-primary focus:ring-offset-2 rounded-2xl p-2"
                        >
                            <div className="relative mb-3">
                                <img
                                    src={profile.avatarUrl}
                                    alt={profile.displayName}
                                    className="w-24 h-24 rounded-full object-cover border-4 border-transparent group-hover:border-ilma-primary transition-colors shadow-lg"
                                />
                                {profile.hasPin && (
                            ...
                            <span className="font-bold text-gray-800 text-sm group-hover:text-ilma-primary transition-colors">
                                {profile.displayName}
                            </span>
                    ...
                    <button
                        ...
                        className="group flex flex-col items-center w-32 transition-transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-ilma-primary focus:ring-offset-2 rounded-2xl p-2"
                    >
                        <div className="w-24 h-24 rounded-full border-2 border-dashed border-gray-300 flex items-center justify-center mb-3 group-hover:border-ilma-primary group-hover:bg-amber-50 transition-colors">
                            <Plus size={32} className="text-gray-400 group-hover:text-ilma-primary" />
                        </div>
                        <span className="font-bold text-gray-500 text-sm group-hover:text-ilma-primary transition-colors">
                            Ajouter
                        </span>
                    </button>
                </div>
